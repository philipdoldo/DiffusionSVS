...
if __name__ == "__main__":
    ...
    test_loader = NaiveDataLoader(data_path=TEST_H5, batch_size=1, padding_value=0, rng_seed=21, diffusion_k=diffusion_k, stats_path=TRAIN_STATS_PATH, diffusion_type=diffusion_type, mel_pad_multiple=mel_pad_multiple, prefetch_batches=0)

    for i in range(len(test_loader.utterances)):
        if i in [0, 27, 49]:
            batch = test_loader.next_batch(device) # maybe create dummy .h5 of just a single example
            print(f"{i=}, {test_loader.utterances[i]=}")
        else:
            batch = test_loader.next_batch("cpu")
            continue
        ...
        if args.ground_truth:
            ground_truth_mel = batch['mel'].cpu().numpy()
            ground_truth_mel = ((ground_truth_mel + 1)/2) * (mel_max - mel_min) + mel_min
            ground_truth_mel_tensor = torch.from_numpy(ground_truth_mel).float().to(device)
        ...
        with torch.no_grad():
            ...
            if args.ground_truth:
                gt_audio = vocoder(ground_truth_mel_tensor, f0_tensor).view(-1)
                gt_audio = gt_audio.cpu().numpy()

        audio = audio.cpu().numpy()
        output_name = f"OUTPUT-{denoiser_checkpoint_path.stem}-{model_type}-batch{i}"
        if args.ema:
            output_name += f"-EMA"
        if use_num_iter:
        ...
        if args.ground_truth:
            gt_output_name = f"GROUND-TRUTH-VOCODER-batch{i}.wav"
            gt_output_path = output_dir / gt_output_name
            sf.write(gt_output_path, gt_audio, samplerate=vocoder_config["audio_sample_rate"])
